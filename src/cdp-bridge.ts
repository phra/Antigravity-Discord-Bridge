...
export class CdpBridge {
    ...
    async extractLastResponseMarkdown(): Promise<string> {
        ...
        // Build script: htmlToMarkdown function + DOM extraction logic
        const extractionLogic = `
            ...
            return { md: '', diag: 'empty:cascade=' + cascadeTag + ',pres=' + preCount + ',children=' + childCount };
        `;
    ...
    private autoAcceptScript = `(() => {
        ...
        function isAcceptButton(text) {
            // Exclude mode toggles first
            if (text === 'Always run' || text.startsWith('Always run')) return false;
            // Match actual approval buttons
            if (text === 'Accept' || text === 'Run') return true;
            if (text.startsWith('Accept'))       return true;  // "Accept all"
            if (text.startsWith('Run'))           return true;  // "RunAlt+⏎", "Run All+1"
